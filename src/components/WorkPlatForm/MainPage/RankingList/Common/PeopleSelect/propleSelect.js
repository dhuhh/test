...
import { Checkbox, Col, message, Row, Select, Spin, Icon } from "antd";
import { GetChannelInfoModel, GetGroupInfoModel } from "$services/newProduct";
import lodash from "lodash";
import styles from "./peopleSelect.less";
import down from "../../down";
import performanceStyles from './index.less';
...
class MultipleSearchInput extends Component {
  ...
  searchChannelInfo = (param, paCurren) => {
    ...
    GetChannelInfoModel(params)
      .then(res => {
        ...
        // 去重
        let setData1 = Array.from(
          new Set(datas.map(item => JSON.stringify(item)))
        ).map(i => JSON.parse(i));
        let setData2 = Array.from(
          new Set(data.concat(datas).map(item => JSON.stringify(item)))
        ).map(i => JSON.parse(i));
  ...
  searchGroupInfo = (param, paCurren) => {
    ...
    GetGroupInfoModel(params)
      .then(res => {
        let len = res.records.length;
        let datas = res.records.map(obj => {
          return {
            chnlCode: obj.grpCode,
            chnlNm: obj.grpNm,
            chnlId: obj.grpId
          };
        });
  ...
  render() {
    ...
    let plaText =
      api === "GetGroupInfoModel" && type === "groupTeam"
        ? "小组代码/小组名称"
    ...
    return (
      ...
      //size={'large'}
        ...
        //   );
        // }}
        className={`${performanceStyles.mulSelect}`}
        showSearch
          ...
          <Row>
            <Spin spinning={loading}>
              <Row type="flex" justify="end">
                <Col style={{ padding: "8px 16px", color: "#244FFF" }}>
                  共{change ? total - 1 : total}条
                </Col>
              </Row>
      ...
      >
        {datas.map(item => (
          <Select.Option
            key={item.chnlCode}
            value={item.chnlId}
            title={`${item.chnlCode}/${item.chnlNm}`}
          >
