...
class SuperSelect extends PureComponent {
  ...
  onChange = (value, opt) => {
    ...
    const { showSearch, onChange, autoClearSearchValue,YXMB } = this.props;
    ...
    if (YXMB === 1 && value.length > 3) {
      message.warning('添加失败-业务开通最多选择三个！')
    } else if (YXMB === 2 && value.length > 5) {
      message.warning('添加失败-理财产品销售最多选择五个！')
    } else if (YXMB === 3 && value.length > 5) {
      message.warning('添加失败-投顾产品签约最多选择五个！')
    }else{
      this.setState({ value });
  ...
  onSearch = (v) => {
    ...
    console.log(showSearch, onSearch, filterOption, children);
  ...
  render() {
    ...
    return (
      <Select
        ...
        className='dropInClassName2'
        //menuItemSelectedIcon = {<img src={checked} style={{width:'16px'}}/>}
