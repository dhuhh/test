...
function SearchForm(props) {
  ...
  // 组件挂载后查询事件数
  useEffect(() => {
    props.calendar && props.queryCalContent();
  ...
  }, [props.calendar]); // props.type, props.custRank, props.sort
  ...
  const handleSubmit = useCallback((flag = 0) => {
    ...
    props.calendar && props.queryBackLogList({ pagination: { current: 1, pageSize: 20, total: -1, paging: 1 } }).then((response) => {
