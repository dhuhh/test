...
class AgentOpenedItem extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      type: false,
  ...
  componentDidMount() {
    this.titleRef?.addEventListener('mouseover', () => { this.subTitleRef.style.color = '#1A2243'; });
    this.titleRef?.addEventListener('mouseout', () => this.subTitleRef.style.color = '#959CBA');
  }

  componentWillUnmount() {
    this.titleRef?.removeEventListener('mouseover', () => { this.subTitleRef.style.color = '#1A2243'; });
    this.titleRef?.removeEventListener('mouseout', () => this.subTitleRef.style.color = '#959CBA');
  }
      ...
      <React.Fragment>
        <div>
          <Row type="flex" justify='space-between' align='middle' style={{ padding: '12px 0', margin: '0 24px', borderBottom: this.props.length - 1 !== this.props.index && '1px solid #EAEEF2' }}>
            <Col><div onClick={this.handleChange} ref={el => this.titleRef = el} className={workStyles.titleHover} style={{ color: "#1A2243", fontWeight: 600, fontSize: 16 }}>{item.motName || '--'}</div><div ref={(el) => this.subTitleRef = el} style={{ color: '#959CBA', fontWeight: 400, fontSize: 12 }}>PC推送{this.state.checked ? '开启' : '关闭'}、{this.state.cusLvl === '0' ? '全部客户' : 'V5-V7金桂卡'}</div></Col>
            <Col onClick={this.handleChange} className={workStyles.titleHover} style={{ cursor: 'pointer', color: '#61698C', fontSize: 12, display: 'flex', alignItems: 'center' }}>
...
export default AgentOpenedItem;
