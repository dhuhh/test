...
//   }
//   componentDidMount(){
//     QueryMarketMaterial({ columnId: this.props.type || 11 }).then(res=>{
//       this.setState({
//         data: res.records,
//       });
//     }).catch((error) => {
...
//         </div>
//         <div className='btn'>
//           <div onClick={this.showModal} className='modalBtn'><img src={zysc} alt=''/>展业素材</div>
//         </div>
//         <Modal
//           title={<div style={{ color: '#1A2243',fontSize: 16,fontWeight: 'bold' }}><img style={{ marginRight: 4,width: 18,marginTop: -3 }} src={zyscModal} alt=''/>展业素材(alphat)</div>}
...
class TableBtn extends Component {
  ...
  showModalC4 = (type) => {
    ...
    if(!selectAll && selectedRows.length === 0){
      ...
      if(type === 1){
        url = `${serverName}/bss/ncrm/ncustomer/customerHandle/customerLabels/page/customerDBQ.sdo?allSelected=${selectAll}&allSelectedDatas=${selectedRows.map(item=>item.cusNo).join(',')}&fromToken=${md5}`;
      }else if(type === 2){
        url = `${serverName}/bss/newMsg/page/sendSingle.sdo?msgType=2&id=${selectedRows.map(item=>item.cusNo).join(',')}&sttgCode=CUST_LST&allSelDatas=${selectedRows.map(item=>item.cusNo).join(',')}&allSel=${selectAll}&fromToken=${md5}`;
      }else if(type === 3){
        url = `${serverName}/bss/ncrm/ncustomer/customerHandle/customerServiceLogs/page/customerFW.sdo?allSelected=${selectAll}&allSelectedDatas=${selectedRows.map(item=>item.cusNo).join(',')}&fromToken=${md5}`;
      }
      console.log(url);
      this.setState({
        c4Url: url,
  ...
  // }
  exportApi = ()=>{
    console.log(queryAlphaCusListExport,'2333');
    return queryAlphaCusListExport
  }
  // 生成uuid
  guid = () => {
  ...
  export = (flag) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        ...
        let tableHeaderCodes = columns.map(item => item.dataIndex).filter(item=> item !== 'phone');
        tableHeaderCodes = tableHeaderCodes.filter(item => item !=='netAssets')
        let headerInfo = columns.map(item => typeof item.title === 'string' ? item.title : item.key).filter(item=> item !== '手机号');
        headerInfo = headerInfo.filter(item => item!=='T0股票适配度')
