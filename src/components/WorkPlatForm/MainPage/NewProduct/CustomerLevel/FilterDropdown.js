...
export default forwardRef((props, ref) => {
  ...
  const handleOk = () => {
    const { confirm, fetchData, setCurrent, setCustNm, setPotential, setCustCode, setReceiveBenefit, setNoReceiveBenefit, setPotentialRange } = props;
    // 根据已选条件筛选表格数据
    if (setPotential) {
      let value = selectRowKeys.length === 0 ? undefined : checkAll ? '2' : selectRowKeys.includes('是') ? '1' : '0';
      setPotential(value);
    } else if (setCustNm) {
      setCustNm(selectRowKeys.length === 0 ? undefined : selectRowKeys);
    } else if (setCustCode) {
      setCustCode(selectRowKeys.length === 0 ? undefined : selectRowKeys);
    } else if (setReceiveBenefit) {
      setReceiveBenefit(selectRowKeys.length === 0 ? undefined : selectRowKeys);
    } else if (setNoReceiveBenefit) {
      setNoReceiveBenefit(selectRowKeys.length === 0 ? undefined : selectRowKeys);
    } else if (setPotentialRange) {
      setPotentialRange(selectRowKeys.length === 0 ? undefined : selectRowKeys);
    }
    setTimeout(() => {
      fetchData();
    });
