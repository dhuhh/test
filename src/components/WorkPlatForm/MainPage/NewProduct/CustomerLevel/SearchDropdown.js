...
export default forwardRef((props, ref) => {
  ...
  const handleOk = (e) => {
    const { confirm, fetchData, setCurrent, setCustNm, setCustCode } = props;
    // 根据已选条件筛选表格数据
    if (setCustNm) {
      setCustNm(inputValue.length === 0 ? undefined : [inputValue]);
    } else if (setCustCode) {
      setCustCode(inputValue.length === 0 ? undefined : [inputValue]);
    }
    setTimeout(() => {
      fetchData();
    });
  ...
  return (
    <div style={{ width: 280, color: '#1A2243', fontSize: 14 }}>
      ...
      <div style={{ padding: 10, display: 'flex', alignItems: 'center' }}>
        ...
        />
        <Button style={{ height: 30, marginLeft: 10 }} className='m-btn-radius ax-btn-small m-btn-blue' type="button" onClick={handleOk}>搜索</Button>
      </div>
