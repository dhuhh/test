...
class Export extends React.Component {
  ...
  showConfirm = () => {
    ...
    Modal.confirm({
      ...
      cancelText: '取消',
      onOk() {
        const { queryParameter = {}, selectAll = true, selectedRowKeys, displayColumns, exportsType = '' } = _props;
        const tableHeaderCodes = [];
        const headerInfo = [];
        displayColumns.forEach((element) => {
          const { dataIndex, title, key } = element;
          if (dataIndex && dataIndex !== 'cz') {
        ...
        } else if (exportsType === 'channelManagement') {
          action = getChannelInfoExport;
          paramsInfo = {
            ...params,
            channelInfoModel: { ...queryParameter },
          };
        }
        const exportPayload = JSON.stringify(paramsInfo);
        // console.log(exportPayload,'exportPayloadexportPayload');
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <Button className={styles.sBtn} onClick={this.showConfirm}>导出</Button>
