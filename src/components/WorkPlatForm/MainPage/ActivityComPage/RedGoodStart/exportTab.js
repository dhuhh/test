...
import { GetMarketDay, QueryExportAuth } from "$services/activityComPage";
...
class ExportTab extends Component {
  ...
  componentDidMount() {
    if (
      ...
      this.props.keys.keys === "peProductsales"
    ) {
  ...
  dateChange = e => {
    if (e) {
      ...
      GetMarketDay({ dayBeforeCount: num }).then(res => {
        const { data } = res;
        if (data > "20230331") {
          this.props.setQueryDate(moment("20230331"));
        } else {
  ...
  showInModal = types => {
    switch (this.props.keys.keys) {
      ...
      case "emProductsales":
        ...
        } else if (types === "理财产品销售系统外") {
          this.setState({ type: "7" });
        } else if (types === "基金投顾购买量") {
          this.setState({ type: "8" });
        }
        break;
      case "emSigning":
        this.setState({ type: "9" });
        break;
      default:
        this.setState({ type: "" });
        break;
  ...
  render() {
    ...
    console.log("**************redGoodStart", redGoodStart);
    return (
      <Row type="flex" align="middle" style={{ padding: "0px 0px 16px 0px" }}>
        <Col span={24}>
          ...
          {showModalPagetTwo.includes(this.props.keys.keys) &&
            redGoodStart.includes("redGoodStart_InIBAnxiAccount") && (
              <>
                <Button
                  style={{
                  ...
                  className={styles.exportTab}
                  onClick={() => this.showInModal("安信国际端")}
                >
                  安信国际端
                </Button>
                <Button
          ...
          {this.props.keys.keys === "emProductsales" &&
            redGoodStart.includes("redGoodStart_InAnxinProductsales") && (
            <Button
              style={{
                background: "rgba(217,23,27,0.06)",
                color: "#D9171B",
              ...
              className={styles.exportTab}
              onClick={() => this.showInModal("安信国际基金销售量")}
            >
                安信国际销售
            </Button>
          )}
          {this.props.keys.keys === "emProductsales" &&
            redGoodStart.includes("redGoodStart_InProductsaleSysOut") && (
              <>
                <Button
                  style={{
                    background: "rgba(217,23,27,0.06)",
                  ...
                  className={styles.exportTab}
                  onClick={() => this.showInModal("基金投顾购买量")}
                >
                  基金投顾
                </Button>
                <Button
                  ...
                  className={styles.exportTab}
                  onClick={() => this.showInModal("理财产品销售系统外")}
                >
                  理财系统外
                </Button>
              </>
          ...
          {this.props.keys.keys === "emSigning" &&
            redGoodStart.includes("redGoodStart_InAcinonyxData") && (
            <Button
              style={{
                background: "rgba(217,23,27,0.06)",
                color: "#D9171B",
          ...
          {this.props.keys.keys === "emSecurities" &&
            redGoodStart.includes("redGoodStart_IntegralData") && (
            <Button
              style={{
                background: "rgba(217,23,27,0.06)",
                color: "#D9171B",
              ...
              className={styles.exportTab}
              onClick={() => this.showInModal("新开信用博衍杯积分")}
            >
                博衍杯积分
            </Button>
          )}
          ...
          >
            <DatePicker
              ...
              value={typeList ? moment("20230101") : moment("20221212")}
              disabled
            />
            ...
            <DatePicker
              ...
              disabledDate={current =>
                typeList
                  ? (current < moment("20221231").endOf("day") ||
                    current > moment("20230331").endOf("day"))
                  : (current < moment("20221211").endOf("day") ||
                    current > moment("20230331").endOf("day"))
              }
