...
import redgoodstart from "$assets/activityComPage/redgoodstart.png";
import smallredgoodstart from "$assets/activityComPage/small_redgoodstart.png";
import {
...
const {
  activityComPage: {
    exportNewValidCustomer,
    exportMidAffluentCustomer,
    exportNewCreditCustomer,
    exportFinancialProSale,
...
const PartmentDetail = (props)=> {
  ...
  const [authShowExport, setAuthShowExport] = useState(false); // 客户明细导出权限点
  const [adminShowExport, setAdminShowExport] = useState(false); // 新增跟管理员具有导出权限的员工权限点
  ...
  useLayoutEffect(()=>{
    ...
    setQueryDate(paramsStr.queryDate);
    if (paramsStr.staffId !== "") {
      console.log("8999999999999:", paramsStr);
  ...
  const getQueryCustAuth = (id,org) => {
    QueryCustAuth({ staffId: id, orgId: org }).then(res => {
      const { data } = res;
    ...
    // 新增跟管理员具有导出权限的员工权限点
  const getAdminShowExport = () => {
    QueryExportAuth().then(res => {
      const { data } = res;
      let param = data == "1" ? true : false;
      setAdminShowExport(param);
    });
  };
  const jumpRelation = (paramsStr) => {
    console.log("测试无开发关系1", redGoodStart);
    console.log("测试无开发关系2", authShowExport);
    console.log("测试无开发关系3", adminShowExport);
    console.log("测试无开发关系4", redGoodStart.includes("redGoodStart_ToExcel"));
    console.log("测试无开发关系5", redGoodStart.indexOf("redGoodStart_ToExcel") > -1);
    return (
      <div>
        {redGoodStart.includes("redGoodStart_ToExcel") ||
        redGoodStart.indexOf("redGoodStart_ToExcel") > -1 ||
        authShowExport ||
        adminShowExport ? (
            <Link
  ...
  let em_yxhZs =
    "折算前新增有效户总数=证券端新开有效户+实际无效户激活数+IB端新开有效户+安信国际端新开有效户";
  let em_yxhIb =
  ...
  let cu_zdfyZs =
    "新增中端富裕客户总数=证券端新开中端富裕客户+无效户激活转中端+IB端新开中端富裕客户+安信国际端新开中端富裕客户";
  let cu_khsZdfy =
  ...
  let se_rzrqZs =
    "折算后总积分=（个人-存量新开信用账户）+2×（机构-存量新开信用账户）+2×（个人-新开户且开通信用账户）+4×（机构-新开户且开通信用账户）+（存量信用账户激活）+（新开信用账户且开期权账户）+（“博衍杯”全国ETF及期权交易大赛）+（新开北交所权限）";
  let se_clJh =
    "从未发生过信用交易的客户在活动期间发生第一笔信用交易的，均记1分。";
  ...
  let emSecurities_partment = [
    ...
    },
    {
      ...
      render: (text, record) => {
        const params = {
          pageTitle: `分支机构奖项-融资融券-${record.name}`,
          tableType: "emSecurities_person",
          orgId: record.id,
          staffId: "",
          showCheckBox: false,
          showExport: false,
  ...
  let emSecurities_person = [
    ...
    },
    {
      ...
      render: (text, record) => {
        const params = {
          pageTitle: `分支机构奖项-融资融券-${
            record.ryxm ? record.ryxm : "无开发关系"
          }`,
          tableType: "emSecurities_customer",
          orgId: paramsStrs.orgId,
          staffId: record.ryid === "" ? "0" : record.ryid,
          showCheckBox: true,
          showExport: false,
  ...
  let emProductsales_partment = [
    ...
    },
    {
      ...
      render: (text, record) => {
        const params = {
          pageTitle: `分支机构奖项-理财产品销售-${record.name}`,
          tableType: "emProductsales_person",
          orgId: record.id,
          staffId: "",
          showCheckBox: false,
          showExport: false,
  ...
  const fetchData = useCallback(
    (payload = {}) => {
      ...
      if (paramsStr.tableType === "emNewaccount_partment") {
        ...
        setAction(exportNewValidCustomer);
        return QueryNewValidCustomer(prams).then(res => {
          setLoading(false);
          return res;
        });
        //分支机构奖项--新增有效户--员工明细
        ...
        setAction(exportMidAffluentCustomer);
        return QueryMidAffluentCustomer(prams).then(res => {
          setLoading(false);
          return res;
        });
        ...
        setExportPerson(emSecurities_person);
        setExportCustomer(emSecurities_customer);
        setAction(exportNewCreditCustomer);
        return QueryNewCreditCustomer(prams).then(res => {
          setLoading(false);
          return res;
        });
        //分支机构奖项--融资融券--员工明细
      } else if (paramsStr.tableType === "emSecurities_person") {
        prams["summaryType"] = "3";
        prams["orgId"] = paramsStr.orgId;
        prams["sort"] = "rzrqZs_DESC";
        seExParams({
        ...
        setColumns(emSecurities_person);
        setExportCustomer(emCustomers_customer);
        setAction(exportNewCreditCustomer);
        return QueryNewCreditCustomer(prams).then(res => {
          setLoading(false);
        ...
        //分支机构奖项--融资融券--客户明细
      } else if (paramsStr.tableType === "emSecurities_customer") {
        prams["summaryType"] = "4";
        prams["staffId"] = paramsStr.staffId;
        prams["orgId"] = paramsStr.orgId;
        prams["newCreditFrom"] = newCreditFrom;
        seExParams({
          summaryType: "4",
          staffId: paramsStr.staffId,
          orgId: paramsStr.orgId,
        ...
        setColumns(emSecurities_customer);
        setAction(exportNewCreditCustomer);
        return QueryNewCreditCustomer(prams).then(res => {
          setLoading(false);
          return res;
        ...
        setExportPerson(emProductsales_person);
        setExportCustomer(peProductsales_person);
        setAction(exportFinancialProSale);
        return QueryFinancialProSale(prams).then(res => {
          setLoading(false);
          return res;
        });
        //分支机构奖项--理财销售产品--员工明细
      } else if (paramsStr.tableType === "emProductsales_person") {
        prams["summaryType"] = "3";
        prams["orgId"] = paramsStr.orgId;
        prams["sort"] = "xsgm_DESC";
        seExParams({
          summaryType: "3",
          orgId: paramsStr.orgId,
          sort: "xsgm_DESC",
        ...
        setColumns(emProductsales_person);
        setExportCustomer(peProductsales_person);
        setAction(exportFinancialProSale);
        return QueryFinancialProSale(prams).then(res => {
          setLoading(false);
        ...
        // 员工个人奖--新增中端富裕客户--客户明细
      } else if (paramsStr.tableType === "peCustomers_partment") {
        prams["summaryType"] = "4";
        prams["staffId"] = paramsStr.staffId;
        prams["midAffFrom"] = midAffFrom;
        seExParams({
          ...
          <div className={`${styles.basic_box} ${styles.tabs}`}>
            <div>
              <DetailExportTab
                ...
                adminShowExport={adminShowExport}
