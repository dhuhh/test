...
const { ftq } = config;
const { businessStatement: { exportDataActivityPerformance } } = ftq;
class ExportTab extends Component {
  ...
  showConfirm = () => {
    ...
    Modal.confirm({
      ...
      onOk () {
        let action = '';
        action = exportDataActivityPerformance;
        const {  headerName = [] ,tableHeaderCodes: tableHeaderCode,payload: activityPerformanceListModel = {} } = _props;
        const headerInfo = headerName.join(',');
        const tableHeaderCodes = tableHeaderCode.join(',');
        activityPerformanceListModel.pageSize = 100000;
        const exportPayload = JSON.stringify({
          tableHeaderCodes,
          headerInfo,
          activityPerformanceListModel,
        });
  ...
  render () {
    ...
    return (
      <Row style={{ color: '#61698C', padding: '0', fontSize: '16px', marginBottom: '16px' }} type='flex' align='middle'>
        ...
        <Col span={24}>
          <Button onClick={this.showConfirm} className="m-btn-radius m-btn-blue" style={{ float: 'right', marginRight: '0',height: '32px',minWidth: '108px' }} >导出表格</Button>
