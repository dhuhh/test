...
import { getPerformanceDetail } from '$services/businessStatement';
...
class SearchForm extends Component {
  ...
  fetchData= (p)=>{
    ...
    let { department,timeValue,queryType,customerType } = this.state;
    ...
    let payload = { 
      ...
      total: -1,
      totalRows: 0 };
    getPerformanceDetail(payload).then(res=>{
  ...
  resetSearchForm = () => {
    const { parentThis } = this.props;
    this.setState({ searchValue: '',timeType: 'year',department: '',customerType: ['0'],queryType: '4' });
    this.getTimeSelece();
    this.props.form.resetFields();
          ...
          <Col xs={12} sm={12} md={12} lg={12} xl={12} xxl={5}>
            <Form.Item className={`m-form-item m-form-bss-item-p`}  label='客户范围'>
              <Select value={queryType} defaultActiveFirstOption={false} onChange={(e) => this.handleQueryTypeChange(e)}>
                ...
                <Select.Option key='3' value='4'>营业部</Select.Option>
              </Select>
            </Form.Item>
          </Col>
          <Col xs={12} sm={12} md={12} lg={12} xl={12} xxl={5}>
            <Form.Item className={`m-form-item m-form-bss-item-p seleArrow`}  label='关系类型'>
              <Select
                value={customerType}
