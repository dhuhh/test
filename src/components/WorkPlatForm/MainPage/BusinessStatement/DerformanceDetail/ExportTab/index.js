...
const { ftq } = config;
const { businessStatement: { exportDataPerformanceDetail } } = ftq;
class ExportTab extends Component {
  ...
  showConfirm = () => {
    ...
    Modal.confirm({
      ...
      onOk () {
        let action = '';
        action = exportDataPerformanceDetail;
        const {  headerName = [] ,tableHeaderCodes: tableHeaderCode,payload: performanceDetailListModel = {}  } = _props;
        performanceDetailListModel.pageSize = 100000;
        const headerInfo = headerName.join(',');
        const tableHeaderCodes = tableHeaderCode.join(',');
        const exportPayload = JSON.stringify({
          performanceDetailListModel,
          headerInfo,
          tableHeaderCodes,
        });
