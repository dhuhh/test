...
import salary from '$assets/pageLayout/salary_manage.png';
import serve from '$assets/pageLayout/serve_manage.png';
import message from '$assets/pageLayout/message_manage.png';
import jiashicang from '$assets/pageLayout/jiashicang_manage.png';
...
function MaintainManagement() {
  const [resultList, setResultList] = useState([]);
  const [maintainManageTree, setMaintainManageTree] = useState({});
  const iamgeList = [salary, serve, message, jiashicang];

  useEffect(()=>{
    setMaintainManageTree(JSON.parse(sessionStorage.getItem('maintainManageTree')) || {});
  }, [sessionStorage.getItem('maintainManageTree')]);

  useEffect(()=>{
    let cardList = [];
    if(maintainManageTree && maintainManageTree.menu) {
      cardList = maintainManageTree.menu.item;
    }
    // console.log('运维管理=======', cardList);
    let tmpList = [];
    if(cardList.length > 0) {
      cardList.forEach((item, index)=>{
        ...
        tmpList.push({ title: item.title[0].text || '--', image: iamgeList[index % 4], catalog: tmpCatalog, secondMenu });
      });
    }
    ...
    sessionStorage.setItem('maintainUrl', JSON.stringify(totalUrl));
  ...
  useLayoutEffect(() => {
    ...
    const timer = setInterval(() => {
      const box = document.getElementsByClassName('maintainCardBox');
      const card = document.getElementsByClassName('maintainManageCard');
      const content = document.getElementsByClassName('maintainCardContent');
      const secondMenu = document.getElementsByClassName('maintainSecondWordTree');
      if(card.length > 0) {
  ...
  const toDetailPage = (listItem, isTree = false) => {
    return (
      <div className={styles.leftWord} key={listItem.name} style={{ marginTop: isTree ? '' : '16px' }}>
        ...
        >
          <div className={styles.toDetail} onClick={()=>{addTabbar(`${listItem.url}|`);viewSensors('运维管理', '运维管理', listItem.name);}}>
      ...
      <React.Fragment key={listItem.name}>
        <div className={styles.secondName}>{listItem.name}</div>
        {
          listItem.children.map((item, index) => {
            return (
              <div className={`${styles.secondWord} maintainSecondWordTree ${listItem.children.length === 1 ? styles.oneItem : index === 0 ? styles.first : index + 1 === listItem.children.length ? styles.last : styles.other}`} key={item.url}>
  ...
  return (
    <div className={`${styles.box} maintainCardBox`} style={{ visibility: 'hidden' }}>
      {
        resultList.map((item) =>{
          return (
            <div className={`${styles.card} maintainManageCard`} key={JSON.stringify(item.secondMenu)}>
              <div className={styles.title}>{item.title}</div>
              <div className={`${styles.content} maintainCardContent`}>
                {
...
export default MaintainManagement;