...
class FilterColumn extends Component {
  ...
  handleClear = () => {
    ...
    parentThis.props.searchForm.setState({ status: '' });
  }
  //  状态筛选框确定
  handleOk = () =>{
    const { parentThis ,confirm } = this.props;
    const { checkedGroup } = this.state;
    parentThis.props.searchForm.setState({ status: checkedGroup.join(',') },()=>{parentThis.props.searchForm.fetchData();});
    if(confirm){
      confirm();
    }
  }
