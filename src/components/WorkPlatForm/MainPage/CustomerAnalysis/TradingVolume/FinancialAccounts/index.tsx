...
import { FetchQueryTradingFinancialManageAccount } from '$services/customeranalysis';
...
const FinancialAccounts: FC<Props> = (props) => {
  ...
  const fetchData = () => {
    if(allParams.length > 0) {
      ...
      FetchQueryTradingFinancialManageAccount(params).then((response: any) => {
  ...
  const getColumns = () => {
    return [
      ...
      },
      {
        ...
        key: '成交金额(元)',
        sorter: (a: any, b: any) => a.transAmount - b.transAmount,
      },
      {
        title: '手续费(元)',
        dataIndex: 'handlingFree',
        key: '手续费(元)',
        sorter: (a: any, b: any) => a.handlingFree - b.handlingFree,
      },
      {
        title: '申购金额(元)',
        dataIndex: 'subscriptionAmount',
        key: '申购金额(元)',
        sorter: (a: any, b: any) => a.subscriptionAmount - b.subscriptionAmount,
      },
      {
        title: '认购金额(元)',
        dataIndex: 'subscribeAmount',
        key: '认购金额(元)',
        sorter: (a: any, b: any) => a.subscribeAmount - b.subscribeAmount,
      },
      {
        title: '定时定投(元)',
        dataIndex: 'fixedVote',
        key: '定时定投(元)',
        sorter: (a: any, b: any) => a.fixedVote - b.fixedVote,
      },
      {
        title: '赎回金额(元)',
        dataIndex: 'redemption',
        key: '赎回金额(元)',
        sorter: (a: any, b: any) => a.redemption - b.redemption,
      },
      {
        title: '操作',
        dataIndex: 'operation',
  ...
  const btnProps = { type: 7, iskhh: true, total: pagination.total, getColumns: getColumns, param: fetchParams, selectAll: selection.selectAll, selectedRows: selection.selectedRows, md5: md5 };
        ...
        <SearchContent getAllParams={getAllParams} queryData={queryData} controlList={controlList} />
        {
          ...
          <>
            ...
            <Modal
              ...
              footer={null}
            >
              <TradingFlow type={3} cusNo={cusNo} queryValue={props.userBasicInfo.ryid} startDate={fetchParams?.startMonth} endDate={fetchParams?.endMonth} custBelong={fetchParams?.custBelong} />
            </Modal>
            <Modal
              title='客户持仓'
              ...
              footer={null}
            >
              <CustomerPosition cusNo={cusNo} tabList={'开放式基金'} />
            </Modal>
          </>
          )
        }
