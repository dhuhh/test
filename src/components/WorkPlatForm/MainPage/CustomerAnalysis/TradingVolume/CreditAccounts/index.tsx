...
import { FetchQueryTradingCreditAccount } from '$services/customeranalysis';
...
const CreditAccounts: FC<Props> = (props) => {
  ...
  const fetchData = () => {
    if(allParams.length > 0) {
      ...
      FetchQueryTradingCreditAccount(params).then((response: any) => {
  ...
  const getColumns = () => {
    return [
      ...
      {
        title: '普通交易量(元)',
        dataIndex: 'ordinaryVolume',
        key: '普通交易量(元)',
        sorter: (a: any, b: any) => a.ordinaryVolume - b.ordinaryVolume,
      },
      {
        title: '信用交易量(元)',
        dataIndex: 'creditVolume',
        key: '信用交易量(元)',
        sorter: (a: any, b: any) => a.creditVolume - b.creditVolume,
      },
      {
        title: '操作',
        dataIndex: 'operation',
  ...
  const btnProps = { type: 6, iskhh: true, total: pagination.total, getColumns: getColumns, param: fetchParams, selectAll: selection.selectAll, selectedRows: selection.selectedRows, md5: md5 };
