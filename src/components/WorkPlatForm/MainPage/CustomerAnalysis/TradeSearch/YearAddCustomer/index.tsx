...
const DefaultCustomer: FC<Props> = (props) => {
  ...
  const fetchData = () => {
    if(allParams.length > 0) {
      ...
      const params: { [key: string]: any } = {
        ...
        queryType: 4, // 查询类型  查询类型  0|查询 1|潜在 2|日增 3| 日减 4| 年增 5| 年减
        custBelong: getTypeValue('customerAttribution'), // 客户归属
        queryCause: getTypeValue('addReason').join(), // 新增原因
        depart: getTypeValue('accountOpeningDepartment').join(), // 开户营业部
        khh: getTypeValue('excludeCus'), // 剔除非中端富裕客户 0 | 1
        custsource: getTypeValue('custSource'), // 客户来源
        custInfo: getTypeValue('inputBox'), // 客户信息
        startDate: getTypeValue('dateSelection')[0]? Number(getTypeValue('dateSelection')[0].format('YYYYMMDD')) : 19000101, // 开始日期
        endDate: getTypeValue('dateSelection')[1]? Number(getTypeValue('dateSelection')[1].format('YYYYMMDD')) : 30000101, // 结束日期
        paging: 1,
  ...
  const controlList = [
    ...
    { name: 'addReason', selectItem: [{ name: '指定交易和转托管入', value: 1 }, { name: '存量客户资产增长', value: 2 }, { name: '资金转入', value: 3 }, { name: '本年度新开客户', value: 4 }] },
    { name: 'dateSelection', dateLabel: '新增日期' },
