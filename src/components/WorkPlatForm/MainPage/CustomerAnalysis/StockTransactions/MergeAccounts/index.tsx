...
const MergeAccounts: FC<Props> = (props) => {
  ...
  useEffect(() => {
    if(getTypeValue('transactionTime') === '0' && getTypeValue('dateSelection')[0] && Number(getTypeValue('dateSelection')[0].format('YYYYMMDD')) < Number(moment().subtract(1, 'year').format('YYYYMMDD'))) {
      message.info(`近期查询可查询时间区间为：${moment().subtract(1, 'year').format('YYYYMMDD')}--上一交易日的信息!`);
    } else if(getTypeValue('transactionTime') === '1' && getTypeValue('dateSelection')[1] && Number(getTypeValue('dateSelection')[1].format('YYYYMMDD')) > Number(moment().subtract(1, 'year').format('YYYYMMDD'))) {
      message.info(`历史查询可查询时间区间为：归档日${moment().subtract(1, 'year').format('YYYYMMDD')}之前的历史信息!`);
    } else {
      fetchData();
    }
  ...
  const fetchData = () => {
    if(allParams.length > 0) {
      ...
      const params: { [key: string]: any } = {
        ...
        type: 0, // 账户类型 0｜合并账户；1｜普通账户；2｜信用账户
        paging: 1,
        ...
        <SearchContent getAllParams={getAllParams} queryData={queryData} controlList={controlList} hasDateDefault={true} />
        {
          ...
          <>
            ...
            <Modal
              ...
              footer={null}
            >
              <TradingCustomer yrid={props.userBasicInfo.ryid} belong={fetchParams?.belong} stockCode={stockCode} startDate={fetchParams?.startDate} endDate={fetchParams?.endDate} />
            </Modal>
            <Modal
              title='交易流水'
              ...
              footer={null}
            >
              <TradingFlow stockCode={stockCode} queryValue={props.userBasicInfo.ryid} startDate={fetchParams?.startDate} endDate={fetchParams?.endDate} custBelong={fetchParams?.belong} />
            </Modal>
          </>
          )
        }
