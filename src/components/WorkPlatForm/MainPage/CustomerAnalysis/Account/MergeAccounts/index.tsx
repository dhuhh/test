...
import { FetchQueryCusProfitAndLossDetails } from '$services/customeranalysis';
...
const MergeAccounts: FC<Props> = (props) => {
  ...
  const fetchData = () => {
    if(allParams.length > 0) {
      ...
      const params: { [key: string]: any } = {
        ...
        accountType: 1, // 1｜合并账户；2｜普通账户；3｜信用账户
        paging: 1,
      ...
      FetchQueryCusProfitAndLossDetails(params).then((response: any) => {
  ...
  const getColumns = () => {
    return [
      ...
      {
        title: '期初资产(元)',
        dataIndex: 'openingAssets',
        key: '期初资产(元)',
        sorter: (a: any, b: any) => a.openingAssets - b.openingAssets,
      },
      {
        title: '期末资产(元)',
        dataIndex: 'closingAssets',
        key: '期末资产(元)',
        sorter: (a: any, b: any) => a.closingAssets - b.closingAssets,
      },
      {
        title: '盈亏金额(元)',
        dataIndex: 'amount',
      ...
      },
      {
        ...
        dataIndex: 'operation',
        key: '操作',
        render: (text: string, record: any) => <div><span onClick={() => { setCusNo(record.cusNo); setOpenLeftVisible(true); }} style={{ marginRight: '20px', fontSize: 14, color: '#244FFF' }}>交易流水</span><span onClick={() => { setCusNo(record.cusNo); setOpenRightVisible(true) }} style={{ fontSize: 14, color: '#244FFF' }}>客户持仓</span></div>,
      },
    ];
  }
  ...
  const btnProps = { type: 4, total: pagination.total, getColumns: getColumns, param: fetchParams, selectAll: selection.selectAll, selectedRows: selection.selectedRows, md5: md5 };
  ...
  return (
    <>
      <Chart ryid={props.userBasicInfo.ryid} tabKey={props.tabKey} />
        ...
        <SearchContent getAllParams={getAllParams} queryData={queryData} controlList={controlList} />
        {
          ...
          <>
            ...
            <Modal
              ...
              footer={null}
            >
              <TradingFlow cusNo={cusNo} queryValue={props.userBasicInfo.ryid} startDate={fetchParams?.startMonth} endDate={fetchParams?.endMonth} custBelong={fetchParams?.custBelong} />
            </Modal>
            <Modal
              title='客户持仓'
              ...
              footer={null}
            >
              <CustomerPosition cusNo={cusNo} tabList={'股票,开放式基金'} />
            </Modal>
          </>
          )
        }
