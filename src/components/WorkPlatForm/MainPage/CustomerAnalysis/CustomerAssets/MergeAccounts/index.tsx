...
const MergeAccounts: FC<Props> = (props) => {
  ...
  const fetchEchartsData = (type: number) => {
    FetchQueryAssetTrend({ 
      ...
      accountType: 1, // 账户类型 1｜合并账户；2｜普通账户；3｜信用账户
     }).then((response: any) => {
  ...
  const fetchData = () => {
    if(allParams.length > 0) {
      ...
      const params: { [key: string]: any } = {
        ...
        source: Number(getTypeValue('custSource')), // 客户来源
  ...
  const monthDefaultValue = [getTypeValue('transactionTime') === '1' ? moment().subtract(1, 'year').subtract(1, 'month').format('YYYYMM') : moment().subtract(1, 'month').format('YYYYMM'), 
  getTypeValue('transactionTime') === '1' ? moment().subtract(1, 'year').format('YYYYMM') : moment().format('YYYYMM')];
  ...
  const controlList = [
    ...
    { name: 'monthSelection', dateLabel: '月份', isDescribe: true, monthDefaultValue: monthDefaultValue },
    { name: 'assetFlow' },
    { name: 'rankOrder' },
    { name: 'rankingRange' },
    { name: 'interval' },
        ...
        <SearchContent getAllParams={getAllParams} queryData={queryData} controlList={controlList} />
        {
          ...
          <>
            ...
            <Modal
              ...
              footer={null}
            >
              <AssetChange cusNo={cusNo} startDate={fetchParams?.startDate} endDate={fetchParams?.endDate} />
            </Modal>
            <Modal
              title='客户持仓'
