...
class VisitedRoutes extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    const { menuCollapsed: preFlag = false, menuTreeLoaded: preMenuLoaded } = this.props;
    const { menuCollapsed: aftFlag = false, menuTreeLoaded: aftMenuLoaded } = nextProps;
    if (preFlag !== aftFlag || preMenuLoaded !== aftMenuLoaded) {
      this.getMenuSiderWidth(nextProps);
    }
  }
  ...
  onChangeHistory = (e = {}) => {
    setTimeout(() => {
      ...
      tmpl = tmpl.filter((m) => { // 把菜单中没有返回的剔除出去
        ...
        const recentlyVisiteUtils = RecentlyVisiteUtils.mapUrls.length > 0 ? [...RecentlyVisiteUtils.mapUrls, { name: '流程中心', url: '/UIProcessor?Table=WORKFLOW_TOTASKS' }] : [];
        const tempIndex = recentlyVisiteUtils.findIndex((tempItem) => { return tempItem.url === m.substring(0, m.indexOf('|')); });
        if (tempIndex > -1 || m === '/index|' || m.indexOf('/wm/') > -1) {
          return true;
        }
        return false;
      });
  ...
  handleDeleteUrl = (url = '') => {
    ...
    const currentPath = prefixPath + suffixPath;
    // 删除对象缓存
    ...
    recentlyVisitedUrls = recentlyVisitedUrls.filter(m => m !== '/UIProcessor|'); // 过滤掉任务中心首页和livebos的前缀页面
    // 如果关闭的不是当前页签，不用跳转
    if (url === currentPath || currentPath.startsWith(url)) {
    // 跳转到临近的url（优先前一个）
      let tmplHrefs = recentlyVisitedUrls.filter(m => m !== '/index/marketShare|' && m !== '/loading|');
  ...
  getMenuSiderWidth = (props) => { // eslint-disable-line
    const { menuTree = [], menuCollapsed = false, location: { pathname = '' }, menuTreeLoaded, width } = props;
    if (menuTree.length > 0) {
      if (pathname) {
        if (pathname === '/index') {
          this.setState({ siderMenuWidth: menuCollapsed ? 220 : 0 });
          return;
        }
        if (pathname === '/loading') {
          this.setState({ siderMenuWidth: 220 });
          return;
        }
        if (menuTreeLoaded && this.isFirstRender) { // 菜单加载完毕且是第一次渲染
          if (!menuCollapsed) {
            this.setState({ siderMenuWidth: width < 1280 ? 0 : 72 });
          } else {
            this.setState({ siderMenuWidth: 220 });
          }
          ...
          if (urlName) {
            ...
            if (!menuCollapsed) {
              this.setState({ siderMenuWidth: 72 });
            } else {
              this.setState({ siderMenuWidth: 220 });
            }
          }
  ...
  getExtraDropdownMenu = () => {
    ...
    const menu = (
      <Menu>
        <Menu.Item>
          <a onClick={() => { closeTabs(0); }} style={{ display: 'inline-block' }}>
            关闭全部
          </a>
        </Menu.Item>
        <Menu.Item>
          <a onClick={() => { closeTabs(1); }} style={{ display: 'inline-block' }}>
            关闭其他
          </a>
        </Menu.Item>
      </Menu>
  ...
  render() {
    ...
    const { menuTree = [], collapsed, width } = this.props;
    // 容器宽度
    ...
    return (
      tmplUrls.length > 0 && (
        <Affix target={() => document.getElementById('htmlContent')}>
          <Row className="m-row-top" style={{ backgroundColor: '#FFFFFF', margin: width < 1280 ? collapsed ? '0' : '0 -1rem' : '0' }}>
            <Col span={24}>
              <Tabs
                ...
                tabBarExtraContent={
                  <Dropdown overlay={this.getExtraDropdownMenu()}>
                    <a className="ant-dropdown-link m-mcolor-link" style={{ display: 'block', width: '3.333rem' }}>
                      <Icon type="down" style={{ fontSize: '1.586rem', lineHeight: '3.5rem', display: 'block' }} />
                    </a>
                  </Dropdown>
                }
              >
                {
                  tmplUrls.map((item) => {
                    ...
                    if (!name) {
                      const recentlyVisiteUtils = RecentlyVisiteUtils.mapUrls.length > 0 ? [...RecentlyVisiteUtils.mapUrls, { name: '流程中心', url: '/UIProcessor?Table=WORKFLOW_TOTASKS' }] : [];
                      const tempIndex = recentlyVisiteUtils.findIndex((tempItem) => { return tempItem.url === url; });
                      name = tempIndex >= 0 ? recentlyVisiteUtils[tempIndex].name : '';
                      if (url === '/index') {
                        name = '首页';
                      }
                      // 判断是否是投顾
                      ...
                      <Tabs.TabPane
                        ...
                        tab={
                          <React.Fragment>
                            ...
                            {
                              tmplUrls.length > 1 && url !== '/index' && <i className="iconfont icon-close-small" style={{ display: 'inline-block', height: '3.286rem', fontSize: '1rem', marginLeft: '.416rem' }} onClick={() => { this.handleDeleteUrl(url); }} />
                            }
                          </React.Fragment>
                        }
                      />
