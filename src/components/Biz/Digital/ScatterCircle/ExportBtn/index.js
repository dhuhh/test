...
const { api } = config;
const { cusgroupbase: { scatterCircleListExport } } = api;
...
class Exports extends React.Component {
  ...
  showConfirm = () => {
    ...
    Modal.confirm({
      ...
      cancelText: '取消',
      onOk() {
        const { queryParameter = {}, selectAll = false, selectedRowKeys, customerListSeniorAndQuickModel = {}, circleModel = {} /* displayColumns */ } = _props;
        // 客户列表导致只能导出这几个字段 写死 别改(tableHeaderNames中文名得跟es中的字段名一样)
        const tableHeaderCodes = ['customer_name', 'customer_status_name', 'fund_account', 'assets.merge'];
        const tableHeaderNames = ['客户名称', '客户状态', '资金账号', '合并户资产(元)'];
        // const tableHeaderCodes = [];
        // const tableHeaderNames = [];
        // displayColumns.forEach((element) => {
        //   const { dataKey, label } = element;
        //   tableHeaderCodes.push(dataKey);
        //   tableHeaderNames.push(label);
        // });

        // const { quickOrCondition = [], customerQueryType = 1, departmentIds = '', keyword = '', lx = '0', totalAssetsEnd = '', totalAssetsStart = '', stockCode = '', customerGroups = '', customerTags = '', financialProductCode = '', logicOrCondition = [] } = queryParameter;
        // const exportPayload = JSON.stringify({
        //   uuid,
        //   isAllSel: selectAll ? 1 : 0,
        //   unSelectCuscodes: selectedRowKeys.join(','),
        //   customerQueryType,
        //   keyword: keyword ? `${lx}||${keyword}` : '',
        //   stockCode,
        ...
        const exportPayload = JSON.stringify({
          ...
          customerListBasicModel: {
            aggIndicators: queryParameter.aggIndicators,
            attrConditionModels: queryParameter.attrConditionModels,
            customerQueryType: queryParameter.customerQueryType,
            keyword: queryParameter.keyword ? `${queryParameter.lx}||${queryParameter.keyword}` : '',
            stockCode: queryParameter.stockCode,
            financialProductCode: queryParameter.financialProductCode,
            totalAssetsStart: queryParameter.totalAssetsStart && queryParameter.totalAssetsStart !== '' ? queryParameter.totalAssetsStart : queryParameter.totalAssetsStart,
            totalAssetsEnd: queryParameter.totalAssetsEnd && queryParameter.totalAssetsEnd !== '' ? queryParameter.totalAssetsEnd : queryParameter.totalAssetsEnd,
            customerGroups: queryParameter.customerGroups,
            customerTags: queryParameter.customerTags,
            departmentIds: queryParameter.departmentIds,
          },
          customerListSeniorAndQuickModel: {
            ...customerListSeniorAndQuickModel,
            quickOrCondition: queryParameter.quickOrCondition,
            logicOrCondition: queryParameter.logicOrCondition,
          },
          ...circleModel,
          tableHeaderCodes,
          tableHeaderNames,
        ...
        form1.method = 'POST';
        // form 提交路径
        form1.action = scatterCircleListExport;
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <AntdButton className="fcbtn m-btn-border m-btn-border-headColor ant-btn btn-1c" onClick={this.showConfirm}>导出
        </AntdButton>
