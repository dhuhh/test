...
class Exports extends React.Component {
  ...
  showConfirm = () => {
    ...
    Modal.confirm({
      ...
      cancelText: '取消',
      onOk() {
        const { /* queryParameter = {}, */ selectAll = false, selectedRowKeys, displayColumns, cachekey } = _props;
        const tableHeaderCodes = [];
        const tableHeaderNames = [];
        displayColumns.forEach((element) => {
          const { dataKey, label, dataIndex, title } = element;
          if(dataKey && label){
        ...
        const indexOfCode = tableHeaderCodes.indexOf('similarPercent');
          if (indexOfCode > -1) {
            tableHeaderCodes.splice(indexOfCode, 1);
        }
        const indexOfName = tableHeaderNames.indexOf('相似度');
          if (indexOfName > -1) {
            tableHeaderNames.splice(indexOfName, 1);
        }
        const exportPayload = JSON.stringify({
          uuid,
        ...
        form1.method = 'POST';
        // form 提交路径
        form1.action = queryGroupSpreadListExport;
